...
impl Environment {
    ...
    pub fn config_for_v86(&self, objects: &[ObjectLink]) -> Option<sqlx::types::JsonValue> {
        use serde_json::{Map, Value,Number};
        if self.environment_framework != Framework::V86 {
            return None;
        }
        let mut base = Map::new();
        base.insert("memory_size".to_string(), Value::Number(Number::from(64 * 1024 * 1024)));

        {
            let mut buffer = Map::new();
         buffer.insert("url".to_string(), Value::String("seabios.bin".to_string()));
         base.insert("bios".to_string(), Value::Object(buffer));
        }
        {
            let mut buffer = Map::new();
            buffer.insert("url".to_string(), Value::String("vgabios.bin".to_string()));
         base.insert("vga_bios".to_string(), Value::Object(buffer));
    }
        ...
        {
            if *object_role == ObjectRole::Content {
                let mut buffer = Map::new();
                buffer.insert("url".to_string(), Value::String(format!("$CONTENT{object_role_index}")));
                buffer.insert("async".to_string(), Value::Bool(true));
                buffer.insert("fixed_chunk_size".to_string(), Value::Number(Number::from(4*1024*1024)));
                base.insert(match object_role_index {
                    0 => "fda",
                    1 => "fdb",
                    2 => "hda",
                    3 => "hdb",
                    4 => "cdrom",
                    _ => {
                        tracing::warn!("Unrecognized object role index {object_role_index} obtaining config for env {} from {link:?}",self.environment_id);
                        "cdrom"
                    }, 
                }.to_string(),
                            Value::Object(buffer)
        ...
        if let Some(Value::Object(cfg)) = &self.environment_config {
            merge_json(&mut base,cfg)
        }
        Some(Value::Object(base))
    }
}

fn merge_json(left:&mut serde_json::Map<String,serde_json::Value>, right:&serde_json::Map<String,serde_json::Value>) {
    // add missing keys of right to left
    for key in right.keys() {
        match (&mut left[key], &right[key]) {
            (serde_json::Value::Object(left_map), serde_json::Value::Object(right_map)) => {
                merge_json(left_map, right_map);
            },
            // otherwise replace
            (_,rval) => {
                left.insert(key.clone(), rval.clone());
            }
        }
    }
}
