...
CREATE MATERIALIZED VIEW instanceWork (row_num, work_id, work_name, work_version, work_platform, instance_id)
  AS SELECT row_number() OVER (ORDER BY f_unaccent(lower(work.work_name)) ASC, f_unaccent(lower(work.work_version)) ASC, f_unaccent(lower(work.work_platform)) ASC, instance.instance_id ASC) - 1,
