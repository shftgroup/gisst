...
impl Task {
    ...
    ) -> Result<Vec<Self>, TimeoutTaskError> {
        sqlx::query_as!(
            Self,
            r#"UPDATE task
...
mod tests {
    ...
    async fn sorting_filtering(pool: PgPool) -> sqlx::Result<()> {
        ...
        assert_eq!(
            Task::get_tasks(conn.as_mut(), Some(TaskState::Done), None)
                .await?
                .len(),
            1
        );
        assert_eq!(
            Task::get_tasks(conn.as_mut(), None, Some("b")).await?.len(),
            2
        );
        assert_eq!(
            Task::get_tasks(conn.as_mut(), Some(TaskState::Error), Some("a"))
                .await?
                .len(),
            2
        );
        let a_claim = Task::claim_available(conn.as_mut(), Some("a"), "test")
            .await?
            .unwrap();
    ...
    async fn updating(pool: PgPool) -> sqlx::Result<()> {
        ...
        let conntest_claim = Task::claim_available(conn.as_mut(), Some("conntest"), "test")
            .await?
            .unwrap();
        assert_eq!(
            Task::claim_available(conn.as_mut(), Some("conntest"), "test").await?,
            None
        );
        assert_eq!(task.task_id, conntest_claim.task_id);
        assert_eq!(
            Task::get_by_id(conn.as_mut(), conntest_claim.task_id)
                .await?
                .unwrap()
                .task_state,
            TaskState::Active
        ...
        for i in 0..TASK_RETRY_LIMIT {
            ...
            assert_eq!(
                Task::get_by_id(conn.as_mut(), task.task_id)
                    .await?
                    .unwrap()
                    .task_state,
                TaskState::Error
    ...
    async fn timeout_stale(pool: PgPool) -> sqlx::Result<()> {
        ...
        let cancelled = Task::timeout_stale(conn.as_mut(), std::time::Duration::from_secs(5))
            .await
            .unwrap();
