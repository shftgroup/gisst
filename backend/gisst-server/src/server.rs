...
pub async fn launch(config: &ServerConfig) -> Result<()> {
    ...
    let rolling_function: Box<dyn Fn(&str, &str) -> RollingFileAppender> =
        match debug_config.log_rolling_type.as_str() {
    ...
    let terminal_layer = fmt::layer().with_target(true).pretty();

    let subscriber = Registry::default()
        .with(env_filter)
        .with(if debug_config.log_to_file {
            Some(file_layer)
        } else {
            None
        })
        .with(if debug_config.log_to_terminal {
            Some(terminal_layer)
        } else {
            None
        });
