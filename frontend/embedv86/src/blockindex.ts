...
export class BlockIndex {
  ...
  public async insert(
    ...
    const hash = await xxhash128(
      new Uint32Array(object.buffer, object.byteOffset, object.byteLength / 4),
    );
    ...
    if (bucket == undefined) {
      ...
      if (
        ...
        this.additions[this.additions.length - 1].when < frame
      ) {
  ...
  public async insert_exact(
    ...
    if (bucket == undefined) {
      ...
      if (
        this.additions.length == 0 ||
        this.additions[this.additions.length - 1].when < when
      ) {
