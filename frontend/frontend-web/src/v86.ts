...
export async function init(ui: UI, embed: EmuControls) {
  ...
  if (embed.start.type == "state") {
    ...
    if (!data.screenshot_id) {
      ...
      const screenshot_data = (await db.getRecordById(
        "screenshot",
        data.screenshot_id,
      )) as GISSTModels.Screenshot;
  ...
  function fill_evtlog(fromidx: number, toidx: number) {
    ...
    ui_state.evtlog_append(
      replay.events.slice(fromidx, toidx).map((evt) => {
        return { t: evt.when, evt };
      }),
    );
  }
  ...
  function zoom_to_fit() {
    ...
    const screen = <HTMLDivElement>(
      ui_state.emulator_div.querySelector("canvas")!.parentElement!
        .parentElement!
    );
  ...
  ui_state.setControl({
    ...
    load_state: (name: string) => {
      (async () => {
        ...
        if (replay_matches) {
          const replay = v86.replays.findIndex(
            (r) => r.id == replay_matches[1],
          );
    ...
    upload_file: (
      ...
      return (async () => {
        if (category == "replay") {
          ...
          const replay_file_promise =
            initial_replay_file &&
            initial_replay_file != GISSTModels.NEVER_UPLOADED_ID
              ? Promise.resolve(initial_replay_file)
              : v86
                  .download_file("replay", file_name)
                  .then((replay) =>
                    db.uploadFile(
                      new File([replay[0]], replay[1]),
                      initial_replay_file,
                      (_pct: number) => {},
                    ),
                  );
          const video_file_promise =
            rec.video_id && rec.video_id != GISSTModels.NEVER_UPLOADED_ID
              ? db
                  .getRecordById("video", rec.video_id)
                  .then((video_rec) => (video_rec as GISSTModels.Video).file_id)
              : v86
                  .download_file("video", file_name)
                  .then((video) =>
                    db.uploadFile(
                      new File([video[0]], video[1]),
                      GISSTModels.NEVER_UPLOADED_ID,
                      (_pct: number) => {},
                    ),
                  );
          const [replay_file_id, video_file_id] = await Promise.all([
            replay_file_promise,
            video_file_promise,
          ]);
          const video = await db.uploadRecord(
          ...
          const file_id = await db.uploadFile(
            new File([blob], name),
          ...
          if (category == "state") {
            const screenshot = await db.uploadRecord(
              { screenshot_data: metadata.screenshot.split(",")[1] },
              "screenshot",
            );
  ...
  v86.set_replay_checkpoints_changed(
    (added: StateInfo[], removed: StateInfo[]) => {
      for (const si of removed) {
        ui_state.removeState(si.name);
      }
      for (const si of added) {
        const checkpoint_matches = si.name.match(
          /replay([0-9a-f-]+)-state([0-9]+)/,
        );
        if (checkpoint_matches == null) {
          throw "added checkpoint with bad name format";
        }
        const replay = v86.replays.find(
          (elt) => elt.id == checkpoint_matches[1],
        )!;
