...
function zoom_to_fit() {
  const canv = <HTMLCanvasElement>(
    ui_state.emulator_div.querySelector("canvas")!
  );
  const content = <HTMLDivElement>(
    document.querySelector(".gisst-internal-content-body")!
  );
  const n = Math.min(
    content.clientWidth / base_width,
    content.clientHeight / base_height,
  );
...
export async function init(ui: UI, embed: EmuControls) {
  ...
  ui_state.setControl({
    ...
    checkpoints_of: (replay_name: string) => {
      ...
      const replay = ra_util.replay_info(
        new Uint8Array(RA.module.FS.readFile(replay_file)),
      ).id;
      const checkpoints = [];
      for (const state of Object.keys(seen_states)) {
        const state_replay = ra_util.replay_of_state(
          RA.module.FS.readFile(state_dir + "/" + state),
        )?.id;
    ...
    upload_file: (
      ...
      const data = RA.module.FS.readFile(path + "/" + file_name);
      return (async () => {
        metadata.record.file_id = await db.uploadFile(
          new File([new Uint8Array(data)], file_name),
          metadata.record.file_id,
          (_percentage: number) => {},
        );
        if (category === "state") {
          const screenshot = await db.uploadRecord(
            { screenshot_data: metadata.screenshot },
            "screenshot",
          );
          (metadata.record as GISSTModels.State).screenshot_id = (
            screenshot as GISSTModels.Screenshot
          ).screenshot_id;
          const state = await db.uploadRecord(metadata.record, category);
          (metadata.record as GISSTModels.State).state_id = (
            state as GISSTModels.State
          ).state_id;
        } else if (category === "replay") {
          const replay = await db.uploadRecord(metadata.record, category);
          (metadata.record as GISSTModels.Replay).replay_id = (
            replay as GISSTModels.Replay
          ).replay_id;
          // upload all associated states too
        } else if (category === "save") {
          const save = await db.uploadRecord(metadata.record, category);
          (metadata.record as GISSTModels.Save).save_id = (
            save as GISSTModels.Save
          ).save_id;
          // upload all associated states too
        }
        return metadata;
  ...
  if (embed.start.type == "state") {
    ...
    if (!data.screenshot_id) {
      ...
      entryScreenshot = await db.getRecordById(
        "screenshot",
        data.screenshot_id,
      );
    }
    seen_states[content_base + ".state1"] = (
      entryScreenshot as GISSTModels.Screenshot
    ).screenshot_data;
    ui_state.newState(
      content_base + ".state1",
      (entryScreenshot as GISSTModels.Screenshot).screenshot_data,
      "init",
      data as GISSTModels.StateFileLink,
    );
  }
  let replay: Uint8Array | null;
  ...
  if (movie) {
    ...
    ui_state.newReplay(
      content_base + ".replay1",
      "init",
      data as GISSTModels.ReplayFileLink,
    );
  }
  ui_state.setReplayMode(
    movie
      ? UIReplayMode.Playback
      : embed.embed_options.record_from_start
        ? UIReplayMode.Record
        : UIReplayMode.Inactive,
  );
  window.RA = RA;
  window.UI = ui_state;
  window.DB = db;
  ...
  base_width =
    parseInt(canv.style.width.slice(0, canv.style.width.length - 2)) || 480;
  canv.style.width = `${base_width}px`;
  canv.width = base_width;
  base_height =
    parseInt(canv.style.height.slice(0, canv.style.height.length - 2)) || 360;
  canv.style.height = `${base_height}px`;
...
async function update_replay_state() {
  ...
  const matches = resp.match(
    /GET_CONFIG_PARAM active_replay ([0-9]+) ([0-9]+) ([0-9]+)$/,
  );
  ...
  if (id == "0" || flags == 0) {
    ...
    const mode =
      (flags & BSVFlags.PLAYBACK) != 0
        ? ReplayMode.Playback
        : flags & BSVFlags.RECORDING
          ? ReplayMode.Record
          : ReplayMode.Inactive;
...
function checkChangedStatesAndSaves() {
  ...
  for (const state of states) {
    ...
    if (state.endsWith(".png") || state.includes(".state")) {
      ...
      const state_file = state.endsWith(".png")
        ? state.substring(0, state.length - 4)
        : state;
      ...
      if (!(
        file_exists(RA, state_dir + "/" + png_file) &&
        file_exists(RA, state_dir + "/" + state_file)
      )) {
        continue;
      }
      // console.log("check state file",state);
      const replay = ra_util.replay_of_state(
        RA.module.FS.readFile(state_dir + "/" + state_file),
      );
      ...
      if (!(state in seen_replays)) {
        const replay = ra_util.replay_info(
          new Uint8Array(RA.module.FS.readFile(state_dir + "/" + state)),
        );
