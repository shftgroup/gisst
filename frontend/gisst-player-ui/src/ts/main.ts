...
export class UI {
  ...
  constructor(ui_root: HTMLDivElement, headless: boolean) {
    ...
    this.control = {
      ...
      set_zoom: (_lev: ZoomLevel) => {
        throw "not implemented";
      },
      enter_fullscreen: () => {
        throw "not implemented";
      },
      activate_save: (_save: string) => {
        throw "not implemented";
      },
      create_save: () => {
        throw "not implemented";
      },
      load_state: (_sn: string) => {
        throw "not implemented";
      },
      save_state: () => {
        throw "not implemented";
      ...
      play_replay: (_sn: string) => {
        throw "not implemented";
      },
      download_file: (
        _category: "save" | "state" | "replay",
        _file_name: string,
      ) => {
        throw "not implemented";
      },
    ...
    if (this.headless) {
      const ui_headless_grid = <HTMLDivElement>(
        elementFromTemplates(UITemplateConst.GRID_CONTAINER_HEADLESS)
      );
      this.ui_root.appendChild(ui_headless_grid);
      this.emulator_div = document.createElement("div");
    } else {
      const gisst_header = <HTMLElement>(
        elementFromTemplates(UITemplateConst.GISST_PAGE_DARK_HEADER)
      );
      document.body.prepend(gisst_header);

      const ui_embedded_grid = <HTMLDivElement>(
        elementFromTemplates(UITemplateConst.GRID_CONTAINER_EMBEDDED)
      );

      const emulator_div = elementFromTemplates(
        UITemplateConst.EMULATOR_SINGLE_DIV,
      );
      // Attach emulator div to ui root
      ui_embedded_grid
        .querySelector("#" + UIIDConst.EMU_EMBEDDED_COL)!
        .appendChild(emulator_div);

      // Create emulator control bar
      ui_embedded_grid
        .querySelector("#" + UIIDConst.EMU_CONTROL_BAR_COL)!
        .appendChild(
          elementFromTemplates(UITemplateConst.EMULATOR_CONTROL_BAR),
        );

      // Add object information tabs
      ui_embedded_grid
        .querySelector("#" + UIIDConst.EMU_CONTEXT_DIV)!
        .appendChild(
          elementFromTemplates(UITemplateConst.EMULATOR_OBJECTS_TABS_EMBEDDED),
        );
      this.ui_root.appendChild(ui_embedded_grid);
      this.emulator_div = this.ui_root.querySelector("#canvas_div")!;

      this.ui_root
        .querySelector("#" + UIIDConst.EMU_TOGGLE_MUTE_BUTTON)!
        .addEventListener("click", () => {
          this.control!.toggle_mute();
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_ZOOM_X05_BUTTON)!
        .addEventListener("click", () => {
          this.control!.set_zoom(ZoomLevel.X05);
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_ZOOM_X1_BUTTON)!
        .addEventListener("click", () => {
          this.control!.set_zoom(ZoomLevel.X1);
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_ZOOM_X2_BUTTON)!
        .addEventListener("click", () => {
          this.control!.set_zoom(ZoomLevel.X2);
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_ZOOM_FIT_BUTTON)!
        .addEventListener("click", () => {
          this.control!.set_zoom(ZoomLevel.Fit);
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_ENTER_FULLSCREEN_BUTTON)!
        .addEventListener("click", () => {
          this.control!.enter_fullscreen();
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_SAVE_STATE_BUTTON)!
        .addEventListener("click", () => {
          this.control!.save_state();
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_CREATE_SAVE_BUTTON)!
        .addEventListener("click", () => {
          this.control!.create_save();
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_START_REPLAY_BUTTON)!
        .addEventListener("click", () => {
          this.control!.start_replay();
        });
      this.ui_root
        .querySelector("#" + UIIDConst.EMU_FINISH_REPLAY_BUTTON)!
        .addEventListener("click", () => {
          this.control!.stop_and_save_replay();
        });
    }

    // Configure emulator manipulation toolbar
    this.saves_elt = <HTMLUListElement>(
      document.getElementById("gisst-saves-list")
    );
    this.replay_elt = <HTMLUListElement>(
      document.getElementById("gisst-replays-list")
    );
    ...
    this.evtlog_elt = <HTMLUListElement>(
      document.getElementById("gisst-play-log")
    );
  }
  ...
  newSave(save_file: string, group_key?: string, metadata?: SaveFileLink) {
    ...
    const new_save_list_object = <HTMLDivElement>(
      elementFromTemplates("card_list_object")
    );
    new_save_list_object
      .querySelector(".card-list-object")!
      .setAttribute("id", valid_for_css(save_file));
    const img = <HTMLImageElement>new_save_list_object.querySelector("img");
    img.style.display = "none";
    const save_object_title = <HTMLHeadElement>(
      new_save_list_object.querySelector("h5")
    );
    save_object_title.textContent =
      save_file + (group_key !== undefined ? " - " + group_key : "");
    save_object_title.addEventListener("click", () => {
      console.log("Activate", save_file);
      this.control!["activate_save"](save_file);
    });
    const save_object_description = <HTMLElement>(
      new_save_list_object.querySelector(".card-text")
    );
    save_object_description.textContent = save_file;
    const save_object_timestamp = <HTMLElement>(
      new_save_list_object.querySelector("small")
    );
    save_object_timestamp.textContent = `Created ${new Date().toLocaleDateString("en-US", this.ui_date_format)}`;
    new_save_list_object
      .querySelector(".download-button")!
      .addEventListener("click", () =>
        this.control!.download_file("save", save_file),
      );
      ...
      .addEventListener("click", () => {
        ...
        if (!metadata.editing && !metadata.stored_on_server) {
          if ((metadata.record as Save).save_id != NEVER_UPLOADED_ID) {
            (metadata.record as Save).save_derived_from = (
              metadata.record as Save
            ).save_id;
            (metadata.record as Save).save_id = NEVER_UPLOADED_ID;
          }
          this.control!.upload_file("save", save_file, metadata).then(
            (md: Metadata) => {
    ...
    const gisst_save_tab = <HTMLDivElement>(
      document.getElementById(UI.gisst_saves_list_content_id)
    );
    gisst_save_tab.appendChild(new_save_list_object);
    this.entries_by_name["sv__" + save_file] = document.getElementById(
      valid_for_css(save_file),
    )!;
    ...
    const save_metadata: Metadata = {
      record: {
        ...
        save_derived_from:
          metadata?.save_derived_from || this.current_config.saves[0]?.save_id,
        state_derived_from:
          metadata?.state_derived_from ||
          (this.current_config.start.type === "state"
            ? (this.current_config.start.data! as StateFileLink).state_id
            : null),
        replay_derived_from:
          metadata?.replay_derived_from ||
          (this.current_config.start.type === "replay"
            ? (this.current_config.start.data! as ReplayFileLink).replay_id
            : null),
        creator_id:
          metadata?.creator_id || "00000000-0000-0000-0000-000000000000",
    ...
    if (save_metadata.stored_on_server) {
      this.entries_by_name["sv__" + save_file]
        .querySelector(".bi-cloud-upload")!
        .classList.add("hidden");
      this.entries_by_name["sv__" + save_file]
        .querySelector(".bi-cloud-arrow-up-fill")!
  ...
  newState(
    ...
    img.src = state_thumbnail.startsWith("data:image")
      ? state_thumbnail
      : "data:image/png;base64," + state_thumbnail;
    ...
    const state_object_title = <HTMLHeadElement>(
      new_state_list_object.querySelector("h5")
    );
    state_object_title.textContent =
      state_file + (group_key !== undefined ? " - " + group_key : "");
    const state_object_description = <HTMLElement>(
      new_state_list_object.querySelector(".card-text")
    );
    state_object_description.textContent = state_file;
    const state_object_timestamp = <HTMLElement>(
      new_state_list_object.querySelector("small")
    );
    ...
    new_state_list_object
      .querySelector(".download-button")!
      .addEventListener("click", () =>
        this.control!.download_file("state", state_file),
      );
      ...
      .addEventListener("click", () => {
        ...
        if (!metadata.editing && !metadata.stored_on_server) {
          if ((metadata.record as State).state_id != NEVER_UPLOADED_ID) {
            (metadata.record as State).state_derived_from = (
              metadata.record as State
            ).state_id;
            (metadata.record as State).state_id = NEVER_UPLOADED_ID;
          }
          this.control!.upload_file("state", state_file, metadata).then(
            (md: Metadata) => {
    ...
    const gisst_state_tab = <HTMLDivElement>(
      document.getElementById(UI.gisst_states_list_content_id)
    );
    gisst_state_tab.appendChild(new_state_list_object);

    this.entries_by_name["st__" + state_file] = document.getElementById(
      valid_for_css(state_file),
    )!;
    nonnull(this.current_config!);
    ...
    if (state_metadata.stored_on_server) {
      this.entries_by_name["st__" + state_file]
        .querySelector(".bi-cloud-upload")!
        .classList.add("hidden");
      this.entries_by_name["st__" + state_file]
        .querySelector(".bi-cloud-arrow-up-fill")!
  ...
  newReplay(
    replay_file: string,
    group_key?: string,
    metadata?: ReplayFileLink,
  ) {
    const li = <HTMLUListElement>elementFromTemplates("replay_list_item");
    li.querySelector(".replay-list-item")!.setAttribute(
      "id",
      valid_for_css(replay_file),
    );
    li.querySelector(".replay-list-item-replay-name")!.textContent =
      replay_file + (group_key !== undefined ? " - " + group_key : "");
    li.querySelector(".replay-list-item-replay-desc")!.textContent =
      replay_file;

    li.querySelector(".replay-list-item-play-button")!.addEventListener(
      "click",
      () => {
    ...
    li.querySelector(".replay-list-item-download-button")!.addEventListener(
      "click",
      () => {
    ...
    li.querySelector(".replay-list-item-upload-button")!.addEventListener(
      "click",
      () => {
        ...
        if ((metadata.record as Replay).replay_id != NEVER_UPLOADED_ID) {
          (metadata.record as Replay).replay_derived_from = (
            metadata.record as Replay
          ).replay_id;
          (metadata.record as Replay).replay_id = NEVER_UPLOADED_ID;
        }
        this.control!.upload_file("replay", replay_file, metadata).then(
          (md: Metadata) => {
            this.completeUpload("rp__" + replay_file, md);
            for (const state_file of this.control!.checkpoints_of(
              replay_file,
            )) {
              const smetadata = this.metadata_by_name["st__" + state_file];
              if (!smetadata) {
                console.warn(
                  "Missing checkpoint metadata for",
                  state_file,
                  "could it be an implicit checkpoint that should not have been returned from checkpoints_of?",
                );
                continue;
              }
              ...
              if (smetadata.editing) {
                this.toggleEditState(state_file);
              }
              this.control!.upload_file("state", state_file, smetadata).then(
                (smd: Metadata) => {
    ...
    li.querySelector(".replay-list-item-edit-button")!.addEventListener(
      "click",
      () => {
        this.toggleEditReplay(replay_file);
      },
    ...
    this.entries_by_name["rp__" + replay_file] = document.getElementById(
      valid_for_css(replay_file),
    )!;
  ...
  toggleEditReplay(replay_file: string) {
    ...
    const replay_list_object = <HTMLDivElement>(
      this.entries_by_name["rp__" + replay_file]
    );
    ...
    if (!replay_metadata.editing) {
      ...
      for (const field of replay_fields) {
        if (canEdit("replay", field.field_name)) {
          const field_element: HTMLParagraphElement =
            document.createElement("p");
          field_element.classList.add(
            valid_for_css(replay_file) + "-edit-fields",
          );
          ...
          if (field.value_type === "string") {
            ...
            const input_element: HTMLInputElement = field_element.querySelector(
              "#" + ele_id,
            )!;
            input_element.value = <string>(
              (replay_metadata.record as Replay)[
                field.field_name as keyof Replay
              ]
            );
            input_element.addEventListener("change", (e: Event) => {
              (replay_metadata.record as Replay)[field.field_name] = (
          ...
          replay_list_object
            ...
            .appendChild(field_element);
        }
      }
    } else {
      replay_metadata.editing = false;
      replay_list_object.querySelector(".card-title")!.textContent =
        (replay_metadata.record as Replay).replay_name +
        (replay_metadata.group_key !== undefined
          ? " - " + replay_metadata.group_key
          : "");
      replay_list_object.querySelector(".card-text")!.textContent = (
        replay_metadata.record as Replay
      ).replay_description;
      const edit_fields = replay_list_object.querySelectorAll(
        "." + valid_for_css(replay_file) + "-edit-fields",
      )!;
  ...
  toggleEditState(state_file: string) {
    ...
    const state_list_object = <HTMLDivElement>(
      this.entries_by_name["st__" + state_file]
    );
    ...
    if (!state_metadata.editing) {
      ...
      for (const field of state_fields) {
        if (canEdit("state", field.field_name)) {
          const field_element: HTMLParagraphElement =
            document.createElement("p");
          field_element.classList.add(
            valid_for_css(state_file) + "-edit-fields",
          );
          ...
          if (field.value_type === "string") {
            ...
            input_element.value = <string>(
              (state_metadata.record as State)[field.field_name as keyof State]
            );
            input_element.addEventListener("change", (e: Event) => {
              (state_metadata.record as State)[field.field_name] = (
                e.currentTarget! as HTMLInputElement
              ).value;
            });
      ...
      state_list_object.querySelector("h5")!.textContent =
        (state_metadata.record as State).state_name +
        (state_metadata.group_key !== undefined
          ? " - " + state_metadata.group_key
          : "");
      state_list_object.querySelector(".card-text")!.textContent = (
        state_metadata.record as State
      ).state_description;
      const edit_fields = state_list_object.querySelectorAll(
        "." + valid_for_css(state_file) + "-edit-fields",
      )!;
      for (let i = 0; i < edit_fields.length; i++) {
        edit_fields[i].remove();
  ...
  toggleEditSave(save_file: string) {
    ...
    const save_list_object = <HTMLDivElement>(
      this.entries_by_name["sv__" + save_file]
    );
    ...
    if (!save_metadata.editing) {
      ...
      for (const field of save_fields) {
        if (canEdit("save", field.field_name)) {
          const field_element: HTMLParagraphElement =
            document.createElement("p");
          field_element.classList.add(
            valid_for_css(save_file) + "-edit-fields",
          );
          ...
          if (field.value_type === "string") {
            ...
            input_element.value = <string>(
              (save_metadata.record as Save)[field.field_name as keyof Save]
            );
            input_element.addEventListener("change", (e: Event) => {
              (save_metadata.record as Save)[field.field_name] = (
                e.currentTarget! as HTMLInputElement
              ).value;
            });
          } else if (field.value_type === "boolean") {
      ...
      save_list_object.querySelector("h5")!.textContent =
        (save_metadata.record as Save).save_short_desc +
        (save_metadata.group_key !== undefined
          ? " - " + save_metadata.group_key
          : "");
      save_list_object.querySelector(".card-text")!.textContent = (
        save_metadata.record as Save
      ).save_description;
      const edit_fields = save_list_object.querySelectorAll(
        "." + valid_for_css(save_file) + "-edit-fields",
      )!;
      for (let i = 0; i < edit_fields.length; i++) {
        edit_fields[i].remove();
...
function elementFromTemplates(template_name: string): Node {
  const templates_element = <HTMLTemplateElement>(
    document.createElement("template")
  );
  templates_element.innerHTML = templates.trim();
  const template_element = <HTMLTemplateElement>(
    templates_element.content.querySelector("#" + template_name)!
  );
