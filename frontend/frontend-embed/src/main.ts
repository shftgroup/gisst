...
async function createContainerUI(container: HTMLDivElement) {
  ...
  container
    .querySelector(`#embed_canvas_${which_canvas}`)!
...
export async function fetchConfig(
  gisst_http_proto: string,
  gisst_root: string,
  gisst_query: string,
) {
  const data_resp = await fetch(
    gisst_http_proto + "://" + gisst_root + "/data/" + gisst_query,
...
export function parseGisstUrl(
  gisst: string,
): [http_proto: string, root: string, query: string] {
  const proto = gisst.slice(0, gisst.indexOf(":"));

  const formatted = gisst
    .replace("/play/", "/")
    .replace("/data/", "/")
    .replace("http:", "gisst:")
    .replace("https:", "gisst:");
  ...
  if (matches) {
    ...
    const same_origin_matches = formatted.match(
      /gisst:\/\/([0-9a-fA-F-]{32,})(\?.+)?$/,
    );
    if (!same_origin_matches) {
      throw "malformed gisst url";
    }
    result_host = document.location.origin.slice(
      document.location.origin.indexOf("://") + 3,
    );
...
export async function embed(
  ...
  const mute_a = container.querySelector(
    "a.gisst-embed-webplayer-mute",
  )! as HTMLLinkElement;
  const halt_a = container.querySelector(
    "a.gisst-embed-webplayer-halt",
  )! as HTMLLinkElement;
  const canvas = container.querySelector(
    "canvas.gisst-embed-webplayer",
  )! as HTMLCanvasElement;
  canvas.style.width =
    container.style.width == "auto" ? "480px" : container.style.width;
  canvas.style.height =
    container.style.height == "auto" ? "360px" : container.style.height;
  ...
  mute_a.addEventListener("click", function () {
    emu.toggle_mute();
  });
  halt_a.addEventListener("click", async function () {
    await emu.halt();
    container.innerHTML = "";
  });
...
function controller_mode_from(s: string | null): ControllerOverlayMode {
  if (s == "on") {
    return ControllerOverlayMode.On;
  } else if (s == "off") {
    return ControllerOverlayMode.Off;
  } else {
    return ControllerOverlayMode.Auto;
  }
}
...
class GISSTElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    embed(src!, div, {
      controls: controller_mode_from(this.getAttribute("controller")),
      record_from_start: false,
      record_video: this.getAttribute("record-video") != null,
    });
  }
}
