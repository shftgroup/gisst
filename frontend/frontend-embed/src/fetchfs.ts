...
export async function registerFetchFS(module:LibretroModule, index:string | Index, root:string, mount:string, cache:boolean) {
  let index_file_tree;
  ...
  for(let i = 0; i < files.length; i+=batch_size) {
    ...
    await Promise.all(file_batch.map(([from,to]) => fetchFile(module,from,to,cache)));
  }
}
...
export async function fetchFile(module:LibretroModule,from:string, to:string, cache:boolean):Promise<void> {
  if (cache) {
    const key = "FSCACHE_"+from;
    const cached = await get(key);
    if (cached) {
      return module.FS.writeFile(to, new Uint8Array(cached));
    } else {
