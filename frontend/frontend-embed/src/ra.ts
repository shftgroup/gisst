...
export async function init(
  ...
  const core = environment.environment_core_name;
  const state_dir = "/mem/states";
  const saves_dir = "/mem/saves";
  const retro_args = ["-v"];
  const content = manifest.find(
    (o) => o.object_role == "content" && o.object_role_index == 0,
  )!;
  const core_entry = core_manifest.find(
    (o) => o.core_role == "entrypoint" && o.core_role_index == 0,
  );
  const core_path = core_entry
    ? "storage/" + core_entry.file_dest_path
    : "cores/" + core + "_libretro.js";
  ...
  for (const o of core_manifest) {
    ...
    if (o.file_filename.match(/.*\.(wasm|js|data)/)) {
      remote_deps[o.file_source_path + o.file_filename] =
        "storage/" + o.file_dest_path;
  ...
  const source_path = content.file_source_path!.replace(
    content.file_filename!,
    "",
  );
  const use_gamepad_overlay =
    embed_options.controls == ControllerOverlayMode.On ||
    ((embed_options.controls ?? ControllerOverlayMode.Auto) ==
      ControllerOverlayMode.Auto &&
  ...
  if (entryState) {
    ...
    state_data = new Uint8Array(
      await (
        await fetch(gisst_root + "/storage/" + data.file_dest_path)
      ).arrayBuffer(),
    );
  }
  ...
  } else if (embed_options.record_from_start) {
    retro_args.push("-R");
    retro_args.push(state_dir + "/" + content_base + ".replay1");
  }
  ...
  for (const save of saves) {
    save_data.push([
      ...
      new Uint8Array(
        await (
          await fetch(gisst_root + "/storage/" + save.file_dest_path)
        ).arrayBuffer(),
      ),
    ]);
  }
  ...
  retro_args.push(
    "/fetch/content/" + source_path + "/" + content.file_filename!,
  );
  console.log(retro_args);
  let ra_cfg_text: string = await (
    await fetch(gisst_root + "/assets/retroarch_web_base.cfg")
  ).text();
  return new Promise((res) => {
    loadRetroArch(
      ...
      remote_deps,
      {
        OPFS_MOUNT: "/home/web_user/retroarch",
        FETCH_MANIFEST: "/mem/fetch.txt",
        FETCH_BASE_DIR: "/fetchfs/",
      },
      true,
      async function (module: LibretroModule) {
        ...
        for (const file of manifest) {
          if (file.object_role == "config") {
            continue;
          }
          let sep = file.file_source_path.startsWith("/") ? "" : "/";
          let download_source_path_full =
            "/fetch/content" + sep + file.file_source_path;
          let download_source_path = download_source_path_full;
          const last_index = download_source_path.lastIndexOf(
            file.file_filename!,
          );
          if (last_index >= 0) {
            download_source_path = download_source_path_full.substring(
              0,
              last_index,
            );
          } else {
        ...
        for (const [data, savefile] of save_data) {
          console.log(
            data,
            "/storage/" + data.file_dest_path,
            saves_dir + "/" + data.save_id + ".srm",
          );
        ...
        if (use_gamepad_overlay) {
          ...
          ra_cfg_text +=
            '\ninput_overlay_enable = "true"\ninput_overlay = "/home/web_user/retroarch/overlays/gamepads/' +
            overlay +
            "/" +
            overlay +
        ...
        const self = {
          ...
          on_ready: () => {},
          inner: {
            send_message(s: string): Promise<void> {
              return send_message(module, s);
            },
            read_response(b: boolean): Promise<string | undefined> {
              return read_response(module, b);
            },
            content_name: content_base,
            state_dir,
            saves_dir,
            module,
          },
        };
        retroReady(module, retro_args, container, self);
        res(self);
      },
    );
  });
...
async function read_response(
  ...
  const waiting: () => Promise<string | undefined> = () =>
    new Promise((resolve) => {
...
async function send_message(module: LibretroModule, msg: string) {
  ...
  while (clearout) {
    clearout = await read_response(module, false);
  }
