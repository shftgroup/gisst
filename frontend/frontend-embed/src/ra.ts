import {ColdStart, StateStart, ReplayStart, CoreFileLink, ObjectLink, Environment, Work, Instance, EmuControls, EmbedOptions, ControllerOverlayMode, SaveFileLink} from './types.d';
import {loadRetroArch,LibretroModule} from './libretro_adapter';

export async function init(gisst_root:string, environment:Environment, work:Work, instance:Instance, start:ColdStart | StateStart | ReplayStart, saves:SaveFileLink[], core_manifest:CoreFileLink[], manifest:ObjectLink[], container:HTMLDivElement, embed_options:EmbedOptions):Promise<EmuControls> {
...
function retroReady(module:LibretroModule, retro_args:string[], container:HTMLDivElement, self:EmuControls): void {
