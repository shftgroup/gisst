...
interface SyncAccessHandle {
  ...
  read(
    ...
    options?: FileSystemReadWriteOptions | undefined,
  ): number;
...
async function writeFile(path: string, data: Uint8Array): Promise<void> {
  ...
  const stream = await (
    file as CanCreateSyncAccessHandle
...
async function setupZipFS(zipBuf: Uint8Array): Promise<void> {
  const zipReader = new zip.ZipReader(new zip.Uint8ArrayReader(zipBuf), {
    useWebWorkers: false,
  });
...
onmessage = async (msg: MessageEvent<SetupMessage>) => {
  if (msg.data.command == "load_bundle") {
    ...
    const resp = await fetch(
      gisst_root + "/assets/frontend/assets_minimal.zip",
      {
        headers: {
