...
const mock_gisst_localhost_url =
  "https://localhost:3000/00000000000000000000000000001036";
const mock_gisst_http_url =
  "https://gisst.pomona.edu/play/5fbf545e-a59f-4556-8626-7bcbb1effd80";
  ...
  manifest: [
    {
      ...
      file_source_path: "",
      file_dest_path:
        "c\\a\\a\\8\\07cd9656778aa01e7f99f37e31b76e24-freedos722.img",
    },
  ],
...
describe("frontend-embed", () => {
  it("should parse localhost url", async () => {
    ...
    expect(fetchUrl).toEqual(
      "https://localhost:3000/data/00000000000000000000000000001036",
    );
  });
  ...
  it("should throw error when server response status is 500", async () => {
    ...
    const [proto, root, query] = parseGisstUrl(mock_gisst_localhost_url);
    await expect(fetchConfig(proto, root, query)).rejects.toThrow(
  ...
  it("should throw error when server response status is 404", async () => {
    ...
    const [proto, root, query] = parseGisstUrl(mock_gisst_localhost_url);
    await expect(fetchConfig(proto, root, query)).rejects.toThrow(
      "Request Status 404: Instance Not Found",
    );
  });
});
