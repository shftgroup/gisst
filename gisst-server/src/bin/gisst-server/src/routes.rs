...
async fn create_image(app_state: Extension<Arc<ServerState>>, mut multipart:Multipart) -> Result<Json<Image>, GISSTError> {
    ...
    if hash.is_some() && Image::get_by_hash(&mut conn, &hash.as_ref().unwrap()).await?.is_none() {
        ...
        if let Ok(file_info) = app_state.storage
            ...
            if let Ok(image) = Image::insert(&mut conn, Image{
                image_id: new_uuid,
                image_hash: hash.unwrap(),
                image_description,
                image_filename: file_info.source_filename,
                image_path: file_info.dest_path,
                created_on: None,
