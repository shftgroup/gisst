...
pub async fn tus_head(
    ...
    if let Some( file) = GFile::get_by_id(&mut conn, id.clone()).await? {
        return Ok(([
            ("Tus-Resumable", "1.0.0"),
    ...
    if let Some(pu) = app_state.pending_uploads.read().unwrap().get(&id) {
        ...
        Ok((StatusCode::NOT_FOUND, format!("Unable to locate pending upload with id {}", id)).into_response())
    }
}
...
pub async fn tus_patch(
    ...
    let mut conn= app_state.pool.acquire().await?;
    if let Some(f) = GFile::get_by_id(&mut conn, id.clone()).await? {
        return Ok(([
    ...
    let mut pu_offset = app_state.pending_uploads.read().unwrap().get(&id).unwrap().offset.clone();
    let pu_length = app_state.pending_uploads.read().unwrap().get(&id).unwrap().length.clone();

    // Check that offset is correct
    if pu_offset != offset {
        return Ok((StatusCode::CONFLICT, format!("Client offset ({}) does not match server offset ({})", pu_offset, offset)).into_response())
    }

    // Check that length is not equal to offset (otherwise there are no bytes to write)
    if pu_offset >= pu_length {
        return Ok((StatusCode::FORBIDDEN, "Upload-Offset is greater than or equal to Upload-Length.").into_response())
    }

    // Check that upload size is not greater than chunk size
    if body.len() > app_state.default_chunk_size {
        return Ok((StatusCode::FORBIDDEN, format!("File is larger than maximum chunk size, ({}) > ({})", body.len(), app_state.default_chunk_size)).into_response())
    }

    // Append chunk to file
    let file_info = app_state.pending_uploads.read().unwrap().get(&id).unwrap().file_information.clone();
    StorageHandler::add_bytes_to_file(
        &app_state.temp_storage_path,
        &file_info,
        body.clone()).await?;
    app_state.pending_uploads.write().unwrap().get_mut(&id).unwrap().offset += body.len();
    pu_offset += body.len();

    // Check if upload is complete and clean up
    if pu_offset == pu_length {
        println!("{}", format!("Got to rename file with the following, temp_path: {}, root_path:{}",
            &app_state.temp_storage_path,
        ...
        GFile::insert(&mut conn, GFile {
            ...
            file_size: pu_length.clone() as i64,
            created_on: None,
        }).await?;
...
pub async fn tus_creation(
    ...
    if metadata.is_none() {
        return Ok((StatusCode::BAD_REQUEST, "Upload-Metadata header is required.").into_response())
    }
        ...
        if metadata.get(&key.to_string()).is_none() {
            return Ok(
                (StatusCode::BAD_REQUEST,
                 format!("Upload-Metadata header must contain a value for '{}' key.", key)).into_response())
        }
    }
    ...
    let file_info = FileInformation {
        ...
        dest_path: StorageHandler::split_uuid_to_path_buf(new_uuid, app_state.folder_depth.clone()).to_string_lossy().to_string(),
    ...
    let _ = &app_state.pending_uploads.write().unwrap().insert(new_uuid, PendingUpload {
    ...
    Ok(([
            ("Tus-Resumable", "1.0.0"),
            ("Location", &format!("/resources/{}", new_uuid.to_string())),
        ],
