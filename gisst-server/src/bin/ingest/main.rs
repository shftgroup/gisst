...
type RetroDB = c_void;
type RetroCursor = c_void;
type RetroQuery = c_void;
type RVal = c_void;

#[allow(dead_code)]
#[link(name = "retro-db")]
extern "C" {
    fn libretrodb_open(path: *const c_char, db: *mut RetroDB) -> c_int;
    fn libretrodb_close(db: *const RetroDB);
    fn libretrodb_free(db: *const RetroDB);
    fn libretrodb_cursor_open(
        db: *const RetroDB,
        cursor: *mut RetroCursor,
        query: *const RetroQuery,
    ) -> c_int;
    fn libretrodb_cursor_read_item(cursor: *const RetroCursor, out: *mut RVal) -> c_int;
    fn libretrodb_cursor_close(cursor: *const RetroCursor);
    fn libretrodb_cursor_free(cursor: *const RetroCursor);
    fn rmsgpack_dom_value_print(obj: *const RVal);
    fn rmsgpack_dom_value_free(obj: *const RVal);

    fn libretrodb_create_index(
        db: *mut RetroDB,
        name: *const c_char,
        field_name: *const c_char,
    ) -> c_int;
    fn libretrodb_find_entry(
        db: *const RetroDB,
