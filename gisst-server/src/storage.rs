...
impl StorageHandler {
    pub fn init(root_storage_path: String, storage_folder_depth: u8) -> StorageHandler {
        if !Path::new(&root_storage_path).is_dir() {
            std::fs::create_dir_all(&root_storage_path)
                .expect("Unable to create root storage directory");
        }
    ...
    pub async fn write_file_to_uuid_folder(
        ...
        Ok(FileInformation {
            ...
            dest_path: path
                .strip_prefix(&self.root_storage_path)?
                .to_string_lossy()
                .to_string(),
...
mod tests {
    ...
    fn depth_from_dest_path() {
        ...
        assert_eq!(
            4,
            StorageHandler::get_folder_depth_from_path(path, Some("somefile.txt".to_string()))
        );
        assert_eq!(
            0,
